        ...
        try {
            ...
            if (problemasDetectados.length === 0) {
                ...
                problemasDetectados.forEach((problema) => {
                    const detallesArr = Array.isArray(problema.detalles) ? problema.detalles : [];
                    const ejemplos = detallesArr.slice(0, 10);
                    const sufijo = detallesArr.length > 10 ? '…' : '';
                    const ejemplosStr = ejemplos.length ? ` | Ejemplos: ${ejemplos.join(', ')}${sufijo}` : '';
                    console.warn(`   • ${problema.tipo} → ${problema.cantidad}${ejemplosStr}`);
                });
                console.warn('ℹ️ Sugerencia: ejecuta window.regenerarAsignacionesDinamicas?.() para intentar reparar automáticamente.');
            }

            return {
                exito: problemasDetectados.length === 0,
            ...
            console.error('❌ [ERROR VALIDACIÓN] Error durante la validación manual:', error);
            return { exito: false, error: error.message };
        }
    }

    // ==================== EXPOSICIÓN DE API EN WINDOW ====================
    try {
        ...
        window.migrarSQLPostImport = migrarSQLPostImport;
        console.log('✅ [EXPORTACIÓN MEJORADA] API expuesta en window');
    } catch (e) {
        console.warn('⚠️ [EXPORTACIÓN MEJORADA] No se pudo exponer API en window:', e);
    }
