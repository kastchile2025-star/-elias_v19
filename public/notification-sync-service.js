...
class NotificationSyncService {
    ...
    saveNotifications(notifications) {
        ...
        try {
            if (notifications.length > HARD_LIMIT) {
                ...
                const archivedPortion = notifications.splice(0, over);
    ...
    _attemptPruneAndSave(notifications, KEY, ARCHIVE_KEY) {
        try {
            ...
            let attempts = 0;
            while (sortable.length > 0 && attempts < 12) {
                attempts++;
            ...
            const fallback = sortable.slice(-50).map(n => ({ id: n.id, t: n.taskTitle, ts: n.timestamp, tp: n.type, crs: n.course }));
            try {
    ...
    _archiveNotifications(items, ARCHIVE_KEY) {
        ...
        try {
            ...
            const minimized = items.map(n => ({ id: n.id, t: n.taskTitle, ts: n.timestamp, tp: n.type, crs: n.course }));
            const merged = existing.concat(minimized).slice(-10000);
    ...
    static repairStoredNotifications() {
        try {
            ...
            let modified = false;
            const getStudentsInCourse = (course)=>{ const list=[]; Object.values(users).forEach(u=>{ if(u.role==='student' && u.activeCourses?.includes(course)) list.push(u.username);}); return list; };
            const validNotifications = [];
            ...
            const MAX_KEEP = 600;
            if (validNotifications.length > MAX_KEEP) { notifications = validNotifications.slice(-MAX_KEEP); modified = true; } else { notifications = validNotifications; }
            if (modified) {
                try { localStorage.setItem(KEY, JSON.stringify(notifications)); console.log(`[NotificationSync][Repair] Reparadas ${notifications.length} notificaciones`); } 
                catch(e){ console.warn('[NotificationSync][Repair] No se pudo guardar reparación:', e.message); try { const minimal = notifications.slice(-200); localStorage.setItem(KEY, JSON.stringify(minimal)); console.log(`[NotificationSync][Repair] Guardadas ${minimal.length} notificaciones mínimas`); } catch (finalError) { console.warn('[NotificationSync][Repair] Error crítico - limpiando storage'); localStorage.removeItem(KEY); } }
            }
        } catch (e) {
