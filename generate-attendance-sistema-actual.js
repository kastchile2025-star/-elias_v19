 ...
 * USANDO LOS DATOS REALES DEL SISTEMA ACTUAL
...
// Estudiantes REALES del sistema actual (según captura de pantalla)
const students = [
  {
    username: 's.gonzalez0008',
    rut: '10000000-8',
    name: 'Sofía González González',
    course: '1ro Básico',
    section: 'A'
  },
  {
    username: 'm.gonzalez0016',
    rut: '10000001-6',
    name: 'Matías González Díaz',
    course: '1ro Básico',
    section: 'A'
  }
];
...
function generateCSV() {
  ...
  console.log('👥 Usando datos del SISTEMA ACTUAL');
  ...
  // Guardar archivo
  const filename = 'asistencia-2-estudiantes-SISTEMA-ACTUAL.csv';
  ...
  console.log('\n📅 Resumen por Semestre:');
  const firstSem = records.filter(r => {
    const month = parseInt(r.date.split('-')[1]);
    return month >= 3 && month <= 6;
  });
  const secondSem = records.filter(r => {
    const month = parseInt(r.date.split('-')[1]);
    return month >= 7 && month <= 12;
  });
  
  console.log(`  📚 Primer Semestre (Marzo-Junio): ${firstSem.length / students.length} días por estudiante`);
  console.log(`  📚 Segundo Semestre (Julio-Diciembre): ${secondSem.length / students.length} días por estudiante`);
}
